...
mcp = FastMCP("MetaApi MCP Server", log_level="DEBUG")
...
def fetch_market(symbol: str, period="1mo", interval="1h", news_top_n=20) -> str:
    ...
    """
    try:
        # Collecte brute via les autres tools
        ...
        news = json.loads(get_ticker_news(symbol))

        # Limiter le nombre de news si besoin
        if isinstance(news, list) and news_top_n:
...
# =========================
@mcp.prompt()
def fetch_market_context(symbol: str, period="1mo", interval="1d") -> str:
    return f"""
...
Étapes :
1) Appelle `get_account_info` .
2) Appelle `get_ticker_news` et conserve titre, publisher, lien, timestamp.
3) Appelle `get_positions` pour récupérer uniquement symbol, type, volume, openPrice, currentPrice, profit, time.
4) Appelle `get_orders` pour récupérer les orders 
5) Appelle `get_symbol_spec` pour récupérer les spécifications du symbole.
6) Appelle `get_current_price` pour récupérer le prix actuel bid/ask.
7) Appelle `get_historical_candles` pour récupérer les bougies historiques.
8) Appelle `get_ticker_news` pour récupérer les news financières.
4) Assemble un **unique objet JSON** propre, strictement au format suivant (n’ajoute rien d’autre) :

⚠️ RÈGLES IMPORTANTES :
- Retourne uniquement du **JSON strictement valide**.
- Pas de texte avant ou après le JSON.
- Pas de commentaires (`/* ... */` ou `// ...`).
- Pas de "..." ni d'abréviations.
- Pas de "..." ni de placeholders (`...TRUNCATED...`, `REDACTED`, etc.).
- Inclue **toutes les données brutes** telles que retournées par les tools, sans les résumer.
- Si une liste est longue, tu dois quand même l'inclure en entier.
- Si la sortie dépasse la taille limite, TRONQUE-TOI correctement en fermant le JSON (pas de placeholder).
...
{{
  ...
  "history": <liste de records OHLCV JSON retournée par get_price_history.data>,
  "news":  <liste JSON retournée par get_ticker_news.data>,
}}

Contraintes :
- Si un tool renvoie {{ "ok": false, ... }}, inclue quand même la clé correspondante avec **null** et ajoute une clé "errors" (liste de messages) dans l’objet racine.
- N’invente aucune valeur. Ne déduis rien. **Aucune** conclusion d’analyse.

Ce paquet JSON est destiné à un **MCP Analysis** séparé qui fera les signaux et recommandations.
