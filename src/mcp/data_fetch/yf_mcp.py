...
def fetch_market_context(
    ...
    """
    Data-only agent prompt: assemble a clean data bundle for downstream analysis.
    The agent MUST NOT produce trading advice here.
    """
    return f"""
Tu es un **agent Data**. Ta mission est **exclusivement** de collecter et assembler les données de marché pour {symbol}.
**NE FOURNIS AUCUNE RECOMMANDATION** (pas de Buy/Sell/Hold). Ne fais **aucune** interprétation.

Tu dois utiliser UNIQUEMENT les tools de ce serveur (Data MCP) :
- get_ticker_info("{symbol}")
- get_ticker_news("{symbol}", top_n={news_top_n})
- get_price_history("{symbol}", period="{period}", interval="{interval}", output_format="json", limit_rows=5000, include_columns="{include_columns}")

Étapes :
1) Appelle `get_ticker_info` puis garde les champs pertinents (secteur, marketCap, ratios clés s’ils existent).
2) Appelle `get_ticker_news` avec `top_n={news_top_n}` et conserve titre, publisher, lien, timestamp.
3) Appelle `get_price_history` avec period="{period}", interval="{interval}", output_format="json", include_columns="{include_columns}".
...
{{
  ...
  "period": "{period}",
  "interval": "{interval}",
  "info": <objet JSON retourné par get_ticker_info.data>,
